...
const TV = ({ tvNumber, name, ip, onSelect, selectedTV, columnId }) => {
  const tvStyle = selectedTV === tvNumber ? { border: "2px solid red" } : {};

  return (
    <div className="tv" style={tvStyle} onClick={() => onSelect(tvNumber, columnId)}>
      <p className="Apps_Screens_Header_Content">{name ? name : `TV${tvNumber}`}</p>
    </div>
  );
};


const Column = ({ id, name, tvs, onAddTV, onSelect, onRemove, onColumnNameChange, selectedTV }) => {
        ...
        >
          <Typography>
            <div className="col-header">
              <input type="text" value={columnName} onChange={handleNameChange} />
            </div>
          </Typography>
        </AccordionSummary>
        <AccordionDetails>
          <div className="tvs-box">
            ...
            <button className="addtv-btn" onClick={() => { onAddTV(); }}>
              +
            </button>
          </div>
        </AccordionDetails>
...
const Apps = () => {
  const [columns, setColumns] = useState([{ id: generateRandomId(8), name: "Column 1", tvs: [] }]);
  ...
  const addColumn = () => {
    ...
    setColumns(prevColumns => {
      const newColumn = { id: generateRandomId(8), name: `Column ${newId}`, tvs: [] };
  ...
  return (
    <div className="Apps_Parent">
      ...
      <div className="leftside columns-container">
        {columns.map((column, index) => (
          <Column
            ...
            tvs={column.tvs}
            onAddTV={() => addTV(index)}
            onSelect={(tvNumber) => handleTVSelect(tvNumber, column.id)} // Pass column ID here
      ...
      <div className="rightside url-box">
        {selectedTV ? <CastingScreensDropdown selectedTV={selectedTV} columns={columns} selectedCol={selectedCol} /> : "Please select a TV"}
      </div>
